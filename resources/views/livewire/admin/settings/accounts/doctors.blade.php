<div>
    <div class="flex gap-4">
        ...
        <x-Essentials.page-title>Doctor Accounts</x-Essentials.page-title>
    </div>
                ...
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 rounded-lg">
                    ...
                    <th scope="row" class="flex items-center px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                        @if (Laravel\Jetstream\Jetstream::managesProfilePhotos())
                            <div class="flex-shrink-0 h-10 w-10"> <img class="h-10 w-10 rounded-full" src="{{ $doctor->profile_photo_url }}" alt="{{ $doctor->name }}"> </div>
                            <div class="ml-4">
                                <div class="text-xs font-medium text-gray-900"> {{ $doctor->first_name .  " " . $doctor->last_name }} </div>
                                <div class="text-xs text-gray-500"> {{ $doctor->email }} </div>
                            </div>
                        @else
                            <div class="ml-4">
                                <div class="text-xs font-medium text-gray-900"> {{ $doctor->name }} </div>
                                <div class="text-xs text-gray-500"> {{ $doctor->email }} </div>
                            </div>
                        @endif
                    </th>
                    ...
                    <td class="px-6 py-6">
                        <span class="{{ $doctor->account_status == false ? 'bg-red-300 text-red-800 text-xs' : 'bg-green-300 text-green-800 text-xs' }} px-2 py-1 rounded-full text-white">
                            {{ $doctor->account_status ? 'Active' : 'Inactive'}}
                        </span>
                    </td>
                    <td class="px-6 py-6 flex gap-2 items-center">
                        <a href="{{route('view-doctor', ['doctor_id' => $doctor->id])}}">
                        ...
                        <svg wire:click='editStatus({{$doctor->id}})' xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 text-gray-400">
                    ...
                    <div class='flex flex-col w-full'>
                        ...
                        <div class='flex gap-4 w-full'>
                            ...
                            <div class='flex flex-col gap-1 mb-4 text-fontColor w-full'>
                                ...
                                <input wire:model="LN" type="text" class="mt-1 focus:ring-green-500 focus:border-green-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
