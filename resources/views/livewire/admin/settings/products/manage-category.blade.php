<div>
    <div class="flex gap-4 items-center">
        <a href="{{route('manage-products')}}">
            <x-Essentials.page-title class="text-[#9D9D9D]">Products</x-Essentials.page-title>
        </a>
        <x-Essentials.page-title class="text-[#9D9D9D]"> > </x-Essentials.page-title>
        <x-Essentials.page-title>Manage Category </x-Essentials.page-title>
    </div>

    {{-- Added Message --}}
    <x-action-message on="category-created" class="w-full text-white bg-green-500 rounded-lg mb-4">
    ...
    {{-- Updated Message --}}
    <x-action-message on="category-updated" class="w-full text-white bg-green-500 rounded-lg mb-4">
        <div class="container flex items-center justify-between px-6 py-4 mx-auto">
            <div class="flex items-center">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6 text-white">
                    <path fill-rule="evenodd" d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12Zm13.36-1.814a.75.75 0 1 0-1.22-.872l-3.236 4.53L9.53 12.22a.75.75 0 0 0-1.06 1.06l2.25 2.25a.75.75 0 0 0 1.14-.094l3.75-5.25Z" clip-rule="evenodd" />
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 sm:rounded-lg shadow-xl py-6 border border-solid">
            ...
            <tbody>
                ...
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 rounded-lg">
                    ...
                    <td class="px-6 py-6">
                        <span class="{{ $category->status == false ? 'bg-red-300 text-red-800 text-xs' : 'bg-green-300 text-green-800 text-xs' }} px-2 py-1 rounded-full text-white">
    ...
    <x-dialog-modal wire:model.live="modalAdd" maxWidth='lg'>
                ...
                <div>
                    <div class='flex gap-4 w-full'>
                        <div class='flex flex-col gap-1 mb-4 text-fontColor w-full'>
                            <x-label for="" value="{{ __('Category Name') }}" />
                            <input wire:model="category_name" type="text" class="mt-1 focus:ring-green-500 focus:border-green-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                            <x-input-error for="category_name"/>
                        </div>
                    </div>
                </div>
            </form>
        ...
        <x-slot name="footer">
            ...
            <x-button class="ms-3" wire:loading.attr="disabled" type='submit' wire:click='createCategory'>
                {{ __('Add') }}
            </x-button>
        </x-slot>
    </x-dialog-modal>
    ...
    <x-dialog-modal wire:model.live="modalUpdate" maxWidth='lg'>
        ...
        <x-slot name="footer">
            ...
            <x-button class="ms-3" wire:loading.attr="disabled" type='submit' wire:click='updateCategory'>
                {{ __('Save') }}
            </x-button>
        </x-slot>
    </x-dialog-modal>
                ...
                <div class="w-full">
                    <div class='flex gap-4 w-full'>
                        <div class='flex flex-col gap-1 mb-4 text-fontColor w-full text-based font-semibold'>
                            <h1>Category ID : <span>{{$category_id}}</span></h1>
                            <h1 class="">Category Name : <span wire:model='category_name'>{{$category_name}}</span></h1>
                            <h1>Status : 
